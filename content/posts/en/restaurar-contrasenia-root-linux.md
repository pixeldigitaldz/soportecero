...
| Cause | Solution |
|---|---|
| **Forgotten or locked root superuser password** | Boot in single-user mode by appending `init=/bin/bash` in GRUB |
| **Read-only mounted filesystem during recovery** | Remount read/write: `mount -o remount,rw /` and reset password via `passwd` |
